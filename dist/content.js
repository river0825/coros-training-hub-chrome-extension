...
(function injectCalendarTab() {
    function waitForTabBarAndInject() {
        ...
        const tabText = '活動列表';
        const tabBar = Array.from(document.querySelectorAll(tabSelector)).find(el => Array.from(el.querySelectorAll('*')).some(child => child.textContent && child.textContent.trim() === tabText));
        if (!tabBar) {
            setTimeout(waitForTabBarAndInject, 500);
            return;
        }
        // Find the "活動列表" tab element
        const activityTab = Array.from(tabBar.querySelectorAll('*')).find(el => el.textContent && el.textContent.trim() === tabText);
        if (!activityTab) {
            setTimeout(waitForTabBarAndInject, 500);
            return;
        }
        // Avoid duplicate injection
        if (tabBar.querySelector('.coros-calendar-tab'))
            return;
      ...
      `;
        ...
        if (!activitiesList)
            activitiesList = tabBar.parentNode;
  ...
  <button id="monthNextBtn" class="arco-btn arco-btn-secondary arco-btn-shape-square arco-btn-size-medium arco-btn-status-normal" type="button"><span class="iconfont iconxiangyou text-14"></span></button>
                ...
                activities.forEach(a => {
                    ...
                    if (a.dateObj.getFullYear() !== year || a.dateObj.getMonth() !== month)
                        return;
                    const sportCodeStr = a.sportCode.toString();
                    if (!report[sportCodeStr])
                        report[sportCodeStr] = { distance: 0, time: 0, count: 0 };
