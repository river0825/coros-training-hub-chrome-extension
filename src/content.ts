...
interface CorosApiResponse {
  data?: {
    dataList?: CorosActivityRaw[];
  };
}

interface ActivityForCalendar extends CorosActivity {
  dateObj: Date;
  type: string;
  sportCode: number;
}
...
interface ActivityMap {
  [date: string]: ActivityForCalendar[];
}
...
interface SportIconEntry {
  icon: string;
  sumSport?: number;
  dataSport: number | string;
  color: string;
}

interface SportIconMap {
  [key: string]: SportIconEntry;
}

// Extend Window interface to include corosActivities
declare global {
  interface Window {
...
(async function extractActivitiesFromAPI(): Promise<void> {
  ...
  const accesstoken = (() => {
    ...
    if (token) {
      console.debug('[content.ts] COROS accesstoken found in cookies.');
    } else {
      console.debug('[content.ts] COROS accesstoken NOT found in cookies.');
    }
    return token;
  })();
  ...
  const url = 'https://teamapi.coros.com/activity/query?size=100&pageNumber=1&modeList=';

  try {
    console.debug('[content.ts] Fetching activities from COROS API:', url);
    ...
    const json: CorosApiResponse = await res.json();
    console.debug('[content.ts] Raw API response:', json);
    const dataList = json?.data?.dataList || [];
    console.debug('[content.ts] Extracted dataList:', dataList);
    ...
    function mapSport(sportType: number): string {
    ...
    const activities: CorosActivity[] = dataList.map(item => ({
    ...
    console.debug('[content.ts] Final extracted activities:', activities);
    window.corosActivities = activities;
  } catch (e) {
    console.error('[content.ts] Failed to fetch activities:', e);
    window.corosActivities = [];
  }
})();

// === Inject "日曆總覽" Tab and Calendar UI ===
(function injectCalendarTab(): void {
  function waitForTabBarAndInject(): void {
    ...
    const calendarTab = activityTab.cloneNode(true) as HTMLElement;
    ...
    let activitiesList = tabBar.parentNode?.querySelector('div,section,main') as HTMLElement;
    if (!activitiesList) activitiesList = tabBar.parentNode as HTMLElement;
    if (activitiesList.parentNode) {
    ...
    let displayedYear: number | undefined;
    let displayedMonth: number | undefined;

    function setDisplayedMonthYear(y: number, m: number): void {
      displayedYear = y;
      displayedMonth = m;
    }

    function getDisplayedMonthYear(): { year: number; month: number } {
    ...
    function showCalendarTab(forceYear?: number, forceMonth?: number): void {
      ...
      const summaryDiv = calendarContainer.querySelector('#coros-calendar-summary') as HTMLElement;
      if (!summaryDiv) return;
      
      summaryDiv.innerHTML = '';
      
      if (window.corosActivities && Array.isArray(window.corosActivities)) {
        const activities: ActivityForCalendar[] = window.corosActivities.map(a => ({
          ...a,
        ...
        function getSportIcon(sportType: number | string): string {
          const entry = sportIconMap[sportType] || sportIconMap['other'];

          console.log('[content.ts] getSportIcon called with:', sportType);
          console.debug('[content.ts] getSportIcon entry:', entry);
          
          const icon = `<span class="arco-table-cell arco-table-cell-align-left"><!----><!----><span class="arco-table-td-content"><div data-v-16496a7c="" class="flex-1 flex"><span data-v-16496a7c="" class="iconfont-sport iconfont-sport ${entry.icon} text-20" data-sport="${entry.dataSport}" style="color: ${entry.color};"></span></div></span></span>`;
          return icon;
        }
        ...
        const monthNavigator = `<div class="arco-btn-group mx-16">
  ...
  <button id="monthNextBtn" class="arco-btn arco-btn-secondary arco-btn-shape-square arco-btn-size-medium arco-btn-status-normal" type="button"><span class="iconfont iconxiangyou text-14"></span></button>
        ...
        for (let d = 1; d <= daysInMonth; d++) {
          ...
          if (activityMap[dateStr]) {
            // Group by type
            const typeGroups: { [type: string]: ActivityForCalendar[] } = {};
            ...
            Object.keys(typeGroups).forEach(type => {
              ...
              function parseTimeToSeconds(t: string | number): number {
              ...
              console.debug('[content.ts] Rendering activity:', type, 'with sportType:', sportTypeValue);
        ...
        activities.forEach(a => {
          ...
          const sportCodeStr = a.sportCode.toString();
          if (!report[sportCodeStr]) report[sportCodeStr] = { distance: 0, time: 0, count: 0 };
          report[sportCodeStr].distance += a.distance || 0;
          report[sportCodeStr].time += parseTimeToSeconds(a.time || 0);
          report[sportCodeStr].count += 1;
          report[sportCodeStr].type = a.type;
        ...
        Object.keys(report).forEach(code => {
          const type = report[code].type || '其他';
        ...
        if (window.$) {
          ...
          window.$('#monthPrevBtn').off('click').on('click', function () {
          ...
          window.$('#monthNextBtn').off('click').on('click', function () {
    ...
    function showActivityTab(): void {
      if (activityTab) {
        setActiveTab(activityTab as HTMLElement);
      }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () {
      console.debug('[content.ts] DOMContentLoaded fired, window.$:', typeof window.$, window.$ ? 'exists' : 'not found');
      waitForTabBarAndInject();
    });
  } else {
    console.debug('[content.ts] Document already loaded, window.$:', typeof window.$, window.$ ? 'exists' : 'not found');
    waitForTabBarAndInject();
  }
})();
